        ...
        <div class="mt-5 flex-1 flex flex-col">
            <nav class="flex-1 px-2 pb-4 space-y-1">
                ...
                <div x-data="{{ request()->is('subjects*') ? '{open:true}' : '{open:false}' }}" class="space-y-1">
                    ...
                    <div x-description="Expandable link section, show/hide based on state." class="space-y-1"
                        ...
                        <a href="{{ route('subjects.create') }}"
                            class="{{ request()->is('subjects/create') ? 'bg-indigo-300 text-black' : '' }} group w-full flex items-center pl-11 pr-2 py-2 text-sm font-medium text-indigo-100 rounded-md hover:text-gray-900 hover:bg-indigo-400">
                            Add a Subject
                        </a>
                ...
                <div x-data="{{ request()->is('students*') ? '{open:true}' : '{open:false}' }}" class="space-y-1">
                    <button type="button"
                        class="text-indigo-100 hover:bg-gray-100 hover:text-gray-900 {{ request()->is('students*') ? 'bg-gray-100 text-black' : '' }} group w-full flex items-center pl-2 pr-1 py-2 text-left text-sm font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        x-state:on="Current" x-state:off="Default" aria-controls="sub-menu-1" @click="open = !open"
                        aria-expanded="open" x-bind:aria-expanded="open.toString()"
                        x-state-description="Current: &quot;bg-gray-100 text-gray-900&quot;, Default: &quot;text-indigo-100 hover:bg-gray-100&quot;">
                        <svg fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"
                    ...
                    <div x-description="Expandable link section, show/hide based on state." class="space-y-1"
                        ...
                        <a href="{{ route('students.create') }}"
                            class="{{ request()->is('students/create') ? 'bg-indigo-300 text-black' : '' }} group w-full flex items-center pl-11 pr-2 py-2 text-sm font-medium text-indigo-100 rounded-md hover:text-gray-900 hover:bg-indigo-400">
                            Add a Student
                        </a>
                        <a href="{{ route('students.index') }}"
                            class="{{ request()->is('students') ? 'bg-indigo-300 text-black' : '' }} group w-full flex items-center pl-11 pr-2 py-2 text-sm font-medium text-indigo-100 rounded-md hover:text-gray-900 hover:bg-indigo-400">
                            Manage Students
                        </a>
                    </div>
                </div>
